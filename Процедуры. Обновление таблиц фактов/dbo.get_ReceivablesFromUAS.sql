...
INNER JOIN (SELECT
				Ссылка AS Ссылка
				,IIF(ДатаОплаты = '1753-01-01', Дата, ДатаОплаты) AS ДатаОплаты
				,ТипДокумента AS ТипДокумента
			FROM [uas_central].dbo.Документ_РеализацияТоваров
			UNION ALL
			SELECT	
				Ссылка AS Ссылка
				,IIF(Дата = '1753-01-01', 0, Дата) AS ДатаОплаты
				,ТипДокумента AS ТипДокумента
			FROM [uas_central].dbo.Документ_ВозвратТоваровОтПокупателя
			UNION ALL
			SELECT			
				Ссылка AS Ссылка
				,IIF(ДатаОплаты = '1753-01-01', IIF(ДатаИсходногоДокумента = '1753-01-01', Дата, ДатаИсходногоДокумента), ДатаОплаты) AS ДатаОплаты
				,ТипДокумента AS ТипДокумента
...
SELECT
	...
	,IIF(SUM(ВзаиморасчетыПоДокументам.Сумма) < 0, 0, ВзаиморасчетыПоДокументам.ДатаОплаты) AS ДатаОплаты
